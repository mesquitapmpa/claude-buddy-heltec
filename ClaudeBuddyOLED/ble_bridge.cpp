...
#include <BLE2902.h>
#include <Arduino.h>
// Core esp32 3.x: ESP32 classico usa Bluedroid, ESP32-S3/C3 usam NimBLE.
// A lib BLE expoe a mesma API de classes nos dois; so a limpeza de bonds
// fala direto com a stack.
...
class RxCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    String v = c->getValue();   // core 3.x: getValue() retorna String
...
#else
  void onMtuChanged(BLEServer*, ble_gap_conn_desc*, uint16_t m) override {
    mtu = m;
    Serial.printf("[ble] mtu=%u\n", mtu);
  }
#endif
...
#endif
  // NimBLE: as flags *_ENCRYPTED nas characteristics ja disparam o pairing
  // no primeiro acesso GATT — nao ha setEncryptionLevel.
...
#else
  // NimBLE: apaga todos os LTKs/CCCDs persistidos de uma vez.
  ble_store_clear();
